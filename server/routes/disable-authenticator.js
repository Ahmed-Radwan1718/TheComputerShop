...
async function checkAttemptLock(uid) {
  const ref = admin.firestore().collection("twoFactorAttempts").doc(uid + "_disable_app");
...
async function clearFailures(uid) {
  await admin.firestore().collection("twoFactorAttempts").doc(uid + "_disable_app").delete().catch(function () {});
}
