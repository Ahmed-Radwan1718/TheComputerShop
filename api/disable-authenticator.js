...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (!existingTwoFactor.appEnabled) {
      return res.status(400).json({ error: "Authenticator app is already disabled." });
    }

    await checkTwoFactorLock(db, decodedUser.uid, "disable_app");
    ...
    if (!verified) {
      const locked = await recordTwoFactorFailure(db, decodedUser.uid, "disable_app");
    ...
    await clearTwoFactorFailures(db, decodedUser.uid, "disable_app");
